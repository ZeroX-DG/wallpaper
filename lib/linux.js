...
const appsList = [
	{
		cmd: "gsettings",
		set: ["set", "org.gnome.desktop.background", "picture-uri", "file://%s"],
		get: ["get", "org.gnome.desktop.background", "picture-uri"],
	...
	{
		cmd: "dcop",
		set: ["kdesktop", "KBackgroundIface", "setWallpaper", "%s 1"]
	},
	{
		cmd: "dconf",
		set: ["write", "/org/mate/desktop/background/picture-filename", '"%s"'],
		get: ["read", "/org/mate/desktop/background/picture-filename"],
...
function setAvailableApps() {
	...
	const whichCmd = `which -a ${names.join(" ")}`;
...
exports.set = function set(imagePath, setApp) {
	if (typeof imagePath !== "string") {
		return Promise.reject(new TypeError("Expected a string"));
	}

	if (!availableApps) {
		return setAvailableApps().then(() => set(imagePath, setApp));
	}

	const app =
		availableApps.find(app => app.set && app.cmd === setApp[0]) ||
		availableApps.find(app => app.set);
	...
	if (app.cmd === "gsettings") {
		if (setApp[1]) {
			if (setApp[1] === "cinnamon") {
				params[1] = "org.cinnamon.desktop.background";
			} else if (setApp[1] === "mate") {
				params.splice(
					1,
					3,
					"org.mate.background",
					"picture-filename",
					params[3].replace(/^file:\/\//, "")
				);
			}
